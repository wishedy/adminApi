...
class Interest{
    ...
    async getSingle(req,res,next){
      const query =  req.query;
      let sendData = {};
      InterestModel.findOne(query, function(error,data) {
        if(error){
          sendData = responseData.createResponseData({
            message: '获取列用户失败',
            data: '',
    ...
    async update(req,res,next){
      const {interest_id, data} =  req.body;
      data.update_time = new Date();
      let sendData = {};
      InterestModel.update({interest_id}, {$set: data}, function(error, data){
        if(error){
          sendData = responseData.createResponseData({
            message:'更新信息失败',
            data: '',
