...
class column {
    ...
    async  createColumn(req,res,next) {
        ...
        if(common.isEmptyObject(paramJson)||common.isNothing(paramJson)){
            ...
            adminModel.findOne({"admin_id":adminId},function(error,adminInfo){
                if(error){
                    sendData = responseData.createResponseData({
                        message:'管理员不存在',
                        data:'NO DATA',
                    ...
                    const  column = columnModel.findOne({'column_title':columnTitle},function(error,columnInfo){
                        if(error){
                            sendData = responseData.createResponseData({
                                message:'添加失败',
                                data:'NO DATA',
    ...
    getJsonList(req,res,next){
        ...
        if(common.isEmptyObject(paramJson)||common.isNothing(paramJson)){
            ...
            let columnFind = ModelData.paginate((searchData), { page: parseInt(pageIndex), limit: parseInt(pageSize) ,populate: {
                    path:'Admin',
                    match: {
                        admin:'5c47cdb36922c83d247b2100'
                    }
                }}, function(error,data){
                if(error){
                    sendData = responseData.createResponseData({
