...
class Article{
    ...
    async changeArticleState(req,res,next){
        let t = this;
        ...
        if(common.isEmptyObject(paramJson)||common.isNothing(paramJson)){
            ...
            let ArticleId = paramJson.articleId;
            ArticleModel.findOne({'article_id':ArticleId},function(error,data){
                if(error){
                    sendData = responseData.createResponseData({
                        message:'查询文章失败',
                        data:'NO DATA',
                            ...
                            if(error){
                                ...
                                if(docs){
                                    ...
                                    switch (parseInt(updateState,10)){
                                        case 1:
                                            sendResponseData(docs,'文章已激活');
                                            break;
                                        case 0:
                                            sendResponseData(docs,'文章已无效');
                                            break;
                                    }
                                }else{
                                    sendData = responseData.createResponseData({
